    ...
    <div style={{ animation: "briefIn 0.5s ease" }}>
      <div style={{ background: T.rD, border: "1px solid rgba(248,113,113,0.2)", borderRadius: 12, padding: 22, marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
          <KV label="Revised Confidence" value={`${da.revised_confidence}/10`} color={(da.revised_confidence ?? 0) >= 7 ? T.g : (da.revised_confidence ?? 0) >= 5 ? T.y : T.rd} />
      ...
      {da.missing_considerations && da.missing_considerations.length > 0 && (
      ...
      {((da.cost_flags?.length ?? 0) > 0 || (da.timeline_flags?.length ?? 0) > 0) && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {da.cost_flags && da.cost_flags.length > 0 && (
          ...
          {da.timeline_flags && da.timeline_flags.length > 0 && (
